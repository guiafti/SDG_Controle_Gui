...
const App: React.FC = () => {
  ...
  const processarCodigo = async (codigo: string) => {
    ...
    try {
      ...
      if (produto) {
        if (produto.stock <= 0) alert(`ATENÇÃO: Produto ${produto.name} sem estoque nesta loja!`);
        setCarrinho(prev => [...prev, { id: produto.id, nome: produto.name, qtd: 1, preco: produto.price, imagem: produto.image }]);
      } else alert('Produto não encontrado no banco local!');
  ...
  const handleMaintenanceSubmit = (aparelho: string, servico: string, valor: number) => {
    setCarrinho(prev => [...prev, { id: 'OS-' + contadorManutencao.toString().padStart(4, '0'), nome: `Serviço: ${aparelho} (${servico})`, qtd: 1, preco: valor, imagem: 'https://placehold.co/150x150/ffedd5/ea580c?text=Serviço' }]);
    setContadorManutencao(prev => prev + 1); setIsMaintenanceOpen(false);
  };
    ...
    <div className="h-screen w-full overflow-hidden">
      ...
      {view === 'pdv' && (
        <div id="tela-pdv" className="fixed inset-0 bg-slate-100 z-50 flex flex-col">
          <PDVHeader loja={loja} vendedor={vendedor} onGoToAdmin={() => setView('admin')} onLogout={handleLogout} />
          <div className="flex-1 overflow-hidden flex gap-6 p-6">
            <div className="flex-1 flex flex-col gap-4">
              <BarcodeScanner onScan={processarCodigo} onOpenMaintenance={() => setIsMaintenanceOpen(true)} />
      ...
      {view === 'admin' && (
        <div id="app-container" className="flex h-screen w-full">
          <Sidebar activeView={adminSubView} onSwitchView={setAdminSubView} onOpenPDV={() => { if (!pdvConfigurado) setIsLoginOpen(true); else setView('pdv'); }} />
          <main className="flex-1 overflow-y-auto bg-slate-50 flex flex-col relative">
            <AdminHeader title={adminSubView === 'dashboard' ? 'Visão Geral (Dashboard)' : adminSubView === 'inventory' ? 'Gestão de Estoque' : 'Comissões'} />
            {renderAdminView()}
          </main>
        </div>
      )}
